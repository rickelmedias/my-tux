...
if ! command -v rocm-smi &>/dev/null; then
  echo "✗ ROCm não encontrado ou reboot não foi feito."
  exit 1
fi
...
# ── Função setup_env (igual ao original) ──────────────────────────────────────
setup_env() {
  local env_name="$1"
  local python_ver="$2"
  local wheel_dir="$3"
  ...
  download_if_missing() {
    ...
    if [[ -f "$decoded_file" ]]; then
      echo "  Wheel já baixado: $decoded_file"
      return 0
    fi
...
# ── Lógica de instalação (com .env ou prompt) ─────────────────────────────────
if [[ -n "$PYTHON_VERSIONS" ]]; then
  ...
  echo "  3.11  - Python 3.11 (experimental)"
  echo "  both  - Ambos"
